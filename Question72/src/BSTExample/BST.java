...
public class BST {
	...
	// Tim cha gan nhat cua 2 node con (su dung de quy)
	public static TreeNode LCAUsingRecurtion(TreeNode root, TreeNode a, TreeNode b) {
		if (root == null)
			return null;
		if (root.data > a.data && root.data > b.data) {
			return LCAUsingRecurtion(root.left, a, b);
		} else if (root.data < a.data && root.data < b.data) {
			return LCAUsingRecurtion(root.right, a, b);
		}
		return root;
	}

	// Tim cha gan nhat cua 2 node con (su dung vong lap)
	public static TreeNode LCAUsingIteration(TreeNode root, TreeNode a, TreeNode b) {
		while (root != null) {
			if (root.data < a.data && root.data < b.data) {
				root = root.right;
			} else if (root.data > a.data && root.data > b.data) {
	...
	public static void main(String[] args) {
		...
		// Tim cha gan nhat cua node 5 va 55
		TreeNode node5 = new TreeNode(5);
		TreeNode node55 = new TreeNode(55);

		TreeNode nodeLca1 = LCAUsingRecurtion(rootNode, node5, node55);
		TreeNode nodeLca2 = LCAUsingIteration(rootNode, node5, node55);

		System.out.println(nodeLca1.data);
		System.out.println(nodeLca2.data);

		// Tim cha gan nhat cua node 5 va 30
		TreeNode node30 = new TreeNode(30);

		TreeNode nodeLca3 = LCAUsingRecurtion(rootNode, node5, node30);
		TreeNode nodeLca4 = LCAUsingIteration(rootNode, node5, node30);

		System.out.println(nodeLca3.data);
		System.out.println(nodeLca4.data);

		// Tim cha gan nhat cua node 60 va 70
		TreeNode node60 = new TreeNode(60);
		TreeNode node70 = new TreeNode(70);

		TreeNode nodeLca5 = LCAUsingRecurtion(rootNode, node60, node70);
		TreeNode nodeLca6 = LCAUsingIteration(rootNode, node60, node70);

		System.out.println(nodeLca5.data);
		System.out.println(nodeLca6.data);
	}
